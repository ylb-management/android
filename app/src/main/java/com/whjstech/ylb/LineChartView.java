...
public class LineChartView extends BaseChartView {
    ...
    public void initData(List<MyPoint> datas) {
        mPoints.clear();
        if (datas != null) {
    ...
    private void drawLine(Canvas canvas) {
        ...
        for (int i = mStartIndex; i <= mStopIndex; i++) {
            MyPoint currentPoint = getItem(i);
            float currentPointX = getX(i);
            MyPoint lastPoint = i == 0 ? currentPoint : getItem(i - 1);
        ...
        if (isLongPress || !isClosePress) {
            MyPoint point = getItem(mSelectedIndex);
            if (point == null) {
                return;
            }
    ...
    private void drawText(Canvas canvas) {
        ...
        if (translateX >= startX && translateX <= stopX) {
            canvas.drawText(DateUtil.getStringDateByLong(mPoints.get(mStartIndex).getDate().getTime(), 7),
                    mTimeLeftPadding,
                    y, mTextPaint);
        }
        translateX = xToTranslateX(mWidth);
        if (translateX >= startX && translateX <= stopX) {
            String text = DateUtil.getStringDateByLong(mPoints.get(mStopIndex).getDate().getTime(), 7);
        ...
        for (int i = 1; i < mLeftCount; i++) {
            ...
            if (translateX >= startX && translateX <= stopX) {
                ...
                String text = DateUtil.getStringDateByLong(mPoints.get(index).getDate().getTime(), 7);
                canvas.drawText(text,
...
//        canvas.drawCircle(stopX, getMainY(stopValue), 5 / mScaleX, mDotPaint);
    ...
    private void drawText(Canvas canvas, int position) {
        ...
        MyPoint point = getItem(position);
        ...
        strings.add(DateUtil.getStringDateByLong(point.getDate().getTime(), 7));
        strings.add("数值");
