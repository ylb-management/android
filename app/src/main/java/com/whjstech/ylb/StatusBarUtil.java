...
public class StatusBarUtil {
    ...
    public static final int MIN_API = 19;/*<editor-fold desc="沉侵">*/
    ...
    public static void immersive(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if ( Build.VERSION.SDK_INT >= 21) {
            ...
            int systemUiVisibility = window.getDecorView().getSystemUiVisibility() | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE;
    ...
    }/*</editor-fold>*//*<editor-fold desc="DarkMode">*/

    public static void darkMode(Activity activity, boolean dark) {
        if (isFlyme4Later()) darkModeForFlyme4(activity.getWindow(), dark);
        else if (isMIUI6Later()) darkModeForMIUI6(activity.getWindow(), dark);
    ...
    public static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        ...
        immersive(window, color, alpha);/*        if (Build.VERSION.SDK_INT >= 21) {*//*            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);*//*            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);*//*            window.setStatusBarColor(Color.TRANSPARENT);*//*        } else if (Build.VERSION.SDK_INT >= 19) {*//*            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);*//*        }*//*        setTranslucentView((ViewGroup) window.getDecorView(), color, alpha);*/
    }/*------------------------->*/
    ...
    private static void darkModeForM(Window window, boolean dark) {/*        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);*//*        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);*//*        window.setStatusBarColor(Color.TRANSPARENT);*/
        int systemUiVisibility = window.getDecorView().getSystemUiVisibility();
        if (dark) systemUiVisibility |= View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR;
        else systemUiVisibility &= ~View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR;
        window.getDecorView().setSystemUiVisibility(systemUiVisibility);
    }
    ...
    /**
     * 设置MIUI6+的状态栏是否为darkMode,darkMode时候字体颜色及icon变黑 http://dev.xiaomi.com/doc/p=4769/
     */
    ...
    public static void setPaddingSmart(Context context, View view) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            ...
            if (lp != null && lp.height > 0) lp.height += getStatusBarHeight(context);/*增高*/
            view.setPadding(view.getPaddingLeft(), view.getPaddingTop() + getStatusBarHeight(context), view.getPaddingRight(), view.getPaddingBottom());
        }
    }
    ...
    private static void setTranslucentView(ViewGroup container, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT >= 19) {
            ...
            if (translucentView != null) translucentView.setBackgroundColor(mixtureColor);
        }
    }
    ...
    private static int getStatusBarHeight(Context context) {
        ...
        if (resId > 0) result = context.getResources().getDimensionPixelSize(resId);
        else
